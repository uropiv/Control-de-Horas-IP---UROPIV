...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  document.getElementById("btn-refresh-all").addEventListener("click", (e) => loadAll(e.currentTarget));
  document.getElementById("btn-search-legajo").addEventListener("click", async () => {
    const btn = document.getElementById("btn-search-legajo");
    const leg = document.getElementById("sup-legajo").value.trim();
    if(!leg) return loadAll(btn);
    const res = await withButtonLoading(
      btn,
      () => api("getUserServices", { token: auth.token, legajo: leg }),
      "Buscando..."
    );
    renderServices(res);
  });
  ...
  document.getElementById("sup-btn-send-suggestion").addEventListener("click", async () => {
    const btn = document.getElementById("sup-btn-send-suggestion");
  ...
  async function loadAll(triggerBtn = null){
    const listCall = () => api("getAllServices", { token: auth.token });
    const res = triggerBtn
      ? await withButtonLoading(triggerBtn, listCall, "Cargando...")
      : await listCall();
    renderServices(res);
  }
  ...
  function renderServices(res){
    ...
    t.style.width = "100%";
    t.style.minWidth = "1520px";
    t.innerHTML = `<thead><tr style="text-align:left"><th>ID</th><th>Legajo</th><th>Tipo</th><th>Subtipo</th><th>Fecha</th><th>Horas</th><th>Obs</th><th>Estado</th><th>Motivo baja</th><th>Baja por</th><th>Fecha baja</th><th>Motivo edicion</th><th>Editado por</th><th>Fecha edicion</th></tr></thead>`;
    const body = document.createElement("tbody");
    res.services.forEach(s=>{
      const tr = document.createElement("tr");
      const estado = s.voided ? "ELIMINADO" : "ACTIVO";
      const fechaBaja = s.void_at ? new Date(s.void_at).toLocaleString() : "";
      const fechaEdicion = s.edited_at ? new Date(s.edited_at).toLocaleString() : "";
      tr.innerHTML = `<td style="padding:6px;border-top:1px solid #eee">${escapeHtml(s.id)}</td>
                      ...
                      <td style="padding:6px;border-top:1px solid #eee">${escapeHtml(s.edit_reason || "")}</td>
                      <td style="padding:6px;border-top:1px solid #eee">${escapeHtml(s.edited_by || "")}</td>
                      <td style="padding:6px;border-top:1px solid #eee">${escapeHtml(fechaEdicion)}</td>`;
      body.appendChild(tr);
    });
    t.appendChild(body);
    c.appendChild(t);
  ...
  if(btnLoadSugs) btnLoadSugs.addEventListener("click", (e) => loadSuggestions(e.currentTarget));

  // Cargar sugerencias (llama al backend)
  async function loadSuggestions(triggerBtn = null){
    const getCall = () => api("getSuggestions", { token: auth.token });
    const res = triggerBtn
      ? await withButtonLoading(triggerBtn, getCall, "Actualizando...")
      : await getCall();
